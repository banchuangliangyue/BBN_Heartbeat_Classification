...
class BBN_ResNet_Cifar(nn.Module):
    def __init__(self, block, num_blocks):
        ...
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 64*2, num_blocks[3], stride=2)
        self.layer5 = self._make_layer(block, 64*4, num_blocks[4] - 1, stride=2)
...
class BBN_ResNet_NSVF(nn.Module):
    def __init__(self, block, num_blocks):
        super(BBN_ResNet_NSVF, self).__init__()
        self.in_planes = 32
        ...
        self.layer5 = self._make_layer(block, 256, num_blocks[4] - 1, stride=2)
        self.cb_block = block(self.in_planes, self.in_planes, stride=1)
        self.rb_block = block(self.in_planes, self.in_planes, stride=1)
...
def bbn_res32_cifar(
    ...
    resnet = BBN_ResNet_NSVF(BasicBlock, [1, 1, 1, 1, 2])
    if pretrain and pretrained_model != "":
        resnet.load_model(pretrain=pretrained_model)
    else:
        print("========Choose to train from scratch=========")
